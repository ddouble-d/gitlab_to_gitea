...
def get_milestones(gitea_api: pygitea, owner: string, repo: string) -> []:
    ...
    if milestone_response.ok:
        existing_milestones = [
            milestone["title"] for milestone in milestone_response.json()
        ]
    else:
        print_error(
...
def get_user_or_group(gitea_api: pygitea, project: gitlab.v4.objects.Project) -> {}:
    result = None
    cleaned_name = name_clean(project.namespace["name"])
    print_info(
        f"Attempting to find user or group for GitLab namespace: '{project.namespace['name']}' (cleaned: '{cleaned_name}')"
    )

    # Try finding as a user first
    user_url = f"/users/{cleaned_name}"
    print_info(f"Checking for user via: {user_url}")
    user_response: requests.Response = gitea_api.get(user_url)
    if user_response.ok:
        print_info(f"Found user '{cleaned_name}'. Status: {user_response.status_code}")
        try:
            result = user_response.json()
            # Gitea might return 200 OK with an empty user object if it's actually an org, check ID
            if result.get("id") == 0:
                print_warning(
                    f"User endpoint returned OK but ID is 0 for '{cleaned_name}', likely an org."
                )
                result = None  # Reset result, force org check
        except json.JSONDecodeError as e:
            print_error(
                f"Failed to decode JSON response for user '{cleaned_name}'. Status: {user_response.status_code}, Error: {e}, Response: {user_response.text}"
            )
            result = None  # Ensure result is None if JSON parsing fails
    else:
        print_warning(
            f"Failed to find user '{cleaned_name}'. Status: {user_response.status_code}, Response: {user_response.text}"
        )

    # If not found as a user (or ID was 0), try finding as an organization
    if result is None:
        org_url = f"/orgs/{cleaned_name}"
        print_info(f"Checking for organization via: {org_url}")
        org_response: requests.Response = gitea_api.get(org_url)
        if org_response.ok:
            print_info(
                f"Found organization '{cleaned_name}'. Status: {org_response.status_code}"
            )
            try:
                result = org_response.json()
            except json.JSONDecodeError as e:
                print_error(
                    f"Failed to decode JSON response for org '{cleaned_name}'. Status: {org_response.status_code}, Error: {e}, Response: {org_response.text}"
                )
                result = None  # Ensure result is None if JSON parsing fails
        else:
            # This is the final error if neither user nor org is found
            print_error(
                f"Failed to find organization '{cleaned_name}'. Status: {org_response.status_code}, Response: {org_response.text}"
            )
            print_error(
                f"Could not find user or organization '{cleaned_name}' in Gitea for GitLab project '{project.name_with_namespace}'."
            )

    if result:
        print_success(
            f"Successfully identified Gitea entity for '{cleaned_name}' (ID: {result.get('id')})"
        )
    else:
        # Explicitly print final failure reason if result is still None
        print_error(
            f"Failed to load user or group '{cleaned_name}' after checking both user and org endpoints."
        )
...
def _import_project_labels(
    ...
    merged_labels = [
        label["name"] for label in get_merged_labels(gitea_api, owner, repo)
    ]
    for label in labels:
...
def _import_project_issues(
    ...
    org_members = []
    members_response = gitea_api.get(f"/orgs/{owner}/members")
    if members_response.ok:
        try:
            members_data = json.loads(members_response.text)
            if isinstance(members_data, list):  # Check if the parsed data is a list
                org_members = [
                    member["login"]
                ...
                print_error(
                    f"Failed to parse org members for {owner}: Expected a list, but got {type(members_data)}"
                )
        except json.JSONDecodeError as e:
            print_error(f"Failed to decode JSON for org members of {owner}: {e}")
            print_error(f"Response text: {members_response.text}")
    else:
        print_error(
            f"Failed to retrieve org members for {owner}: {members_response.status_code} - {members_response.text}"
        )
    ...
    for issue in issues:
        ...
        if not gitea_issue:
            ...
            if (
                ...
                and issue.milestone["title"] in existing_milestones
            ):
        ...
        _import_issue_comments(
            gitea_api, project_id, gitea_issue, owner, repo, notes, org_members
        )
    ...
    for project in projects:
        ...
        try:
            ...
            issues: [gitlab.v4.objects.ProjectIssue] = sorted(
                project.issues.list(all=True), key=lambda x: x.iid
            )
            ...
            # import issues
            _import_project_issues(
                gitea_api, project.id, issues, projectOwner, projectName
            )
